...
struct stmt *parse_funcall(const char **toks)
{
  const char *toks_pre = *toks;
  struct stmt *out = calloc(1, sizeof(*out));
  out->kind = funcall;

  out->funcall.funcexpr = parse_funcval(toks);
  expect (out->funcall.funcexpr);
  expect (streq(next(toks), ";"));
  return out;
...
struct stmt *parse_defunc(const char **toks)
{
  ...
  struct expr *arg;
  L_FOREACH(out->defunc.args->list.exprs, arg)
    expect (arg->kind == var);
...
struct stmt *parse_block(const char **toks)
{
  ...
  struct stmt *out = calloc(1, sizeof(*out));
  out->kind = block;
  out->block.stmts = calloc(1, sizeof(*out->block.stmts));

  expect (streq(next(toks) ,"{"));
  struct stmt *nxt;
  while ((nxt = parse_stmt(toks))) {
    l_push(out->block.stmts, nxt);
  }
  expect (streq(next(toks) ,"}"));
  return out;
